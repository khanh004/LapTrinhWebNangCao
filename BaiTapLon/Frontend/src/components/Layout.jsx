...
export default function Layout({ title, children }) {
  ...
  useEffect(() => {
    if (!hasRole("Admin", "Receptionist")) return;
    const fetchCount = async () => {
      try {
        const res = await waitlistApi.getNotified();
        setNotifiedCount(res.data.length);
      } catch {
        // bỏ qua lỗi lặt vặt khi poll nền
      }
    };
    fetchCount();
          ...
          <div className="flex items-center justify-between py-3.5 border-b border-white/10">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {hasRole("Admin", "Receptionist") && notifiedCount > 0 && (
  <button
    onClick={() => navigate("/reception")}
    className="relative flex items-center gap-1.5 bg-red-50 border border-red-200 text-red-600 text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-red-100 transition-colors"
  >
    <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2">
      <path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M13.73 21a2 2 0 0 1-3.46 0" strokeLinecap="round" />
    </svg>
    {notifiedCount} khách chờ cần gọi
  </button>
)}
